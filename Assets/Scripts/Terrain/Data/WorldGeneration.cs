  ...
  public class WorldGeneration : MonoBehaviour
  {
    ...
    private void GenerateWorld()
    {
      ...
      // CreateIron();
        ...
        public float PerlinCalculation3D(float x, float y, float scaleX = 1, float scaleY = 1, float offset = 0)
    {
      return Mathf.PerlinNoise(x * scaleX + offset, y * scaleY + offset);
    }

    public float PerlinCalculation2D(float x, float terrainHeight, float offset = 0)
    {
    ...
    /// </summary>
		private void CreateOutline()
		{
			for (int x = 0; x < worldWidth * ChunkData.m_Size; x++)
			{
				...
				SetTileAt(x, (int)_elevation, TileType.Grass);
			}
		}
				...
				while (!_finished && y < worldHeight * ChunkData.m_Size)
				{
					...
					if (GetTileAt(x, y) != 0)
					{
						...
						for (int i = y - 1; i > 0; i--)
						{
							...
							else if (y - _elevation == 10)
							{
								_tileToPlaceId = TileType.Gold;
							}
							else if(y - _elevation == 15) { 
								_tileToPlaceId = TileType.Iron;
							}
							else if(y - _elevation == 20) {
								_tileToPlaceId = TileType.Coal;
							}
							else
							{
